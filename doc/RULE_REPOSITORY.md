...
```mermaid
graph LR
    A["GitHub Actions<br>每週一 03:00 UTC"] --> B["下載 AdGuard Base Filter"]
    A --> C["下載 EasyList"]
    A --> D["下載 EasyPrivacy"]
    A --> D2["下載 Peter Lowe's List"]
    B --> E["轉換格式<br>提取域名"]
    C --> E
    D --> E
    D2 --> E
    E --> F["去重排序"]
    F --> G["更新 domains/adsweep_domains.txt"]
    G --> H["更新 index.json count"]
    H --> I["commit + push"]
```

- Workflow: `.github/workflows/update-domains.yml`
- Script: `scripts/update_domains.py`
- 支援手動觸發 (`workflow_dispatch`)
- inject.py `--rules-url auto` 和 Manager PATCH 時自動下載最新域名清單
- 下載失敗時靜默 fallback 到 bundled 版本
...
4. ✅ **域名清單整合** — `domains/adsweep_domains.txt` 放入 adsweep-rules，inject.py 和 Manager 自動下載最新版
5. ✅ **GitHub Actions 自動更新域名清單** — 每週一從 EasyList/AdGuard/EasyPrivacy 同步
6. ✅ **`--discover` 模式**
